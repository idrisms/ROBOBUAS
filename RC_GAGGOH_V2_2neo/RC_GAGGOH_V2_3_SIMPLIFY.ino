...
void loop() {
  ...
  if (Vbat <= Vmin) {
    ...
    Serial.println(" | LOW BATTERY STOP");
  ...
  if (abs(SpeedCh2) < offset) {
    pixels.setPixelColor(0, pixels.Color(0, 30, 0));  // green neutral
  } else if (SpeedCh2 > 0) {
    pixels.setPixelColor(0, pixels.Color(0, 0, 30));  // blue forward
  } else {
    pixels.setPixelColor(0, pixels.Color(30, 0, 0));  // red backward static
  }
  ...
  if (abs(forward) < offset) forward = 0;
  if (abs(turn) < offset) turn = 0;
