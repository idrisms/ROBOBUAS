...
void readTrimmerFW() {
  int trimValue = analogRead(TRIMMER_PIN_FW);
...
void readTrimmerBW() {
  int trimValue = analogRead(TRIMMER_PIN_BW);

  if (trimValue < 512) {
    compensateLeftBW = map(512 - trimValue, 0, 512, 0, 60);
    compensateRightBW = 0;
  } else {
    compensateRightBW = map(trimValue - 512, 0, 511, 0, 60);
    compensateLeftBW = 0;
  }
}
